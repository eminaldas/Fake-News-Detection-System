...
import InfoTip from '../components/features/borsa/InfoTip';

const TIP = {
    ma20: 'MA20: 20 dönemlik hareketli ortalama. Fiyat bunun üstündeyse kısa vadeli yukarı eğilim.',
    ma50: 'MA50: 50 dönemlik hareketli ortalama. Orta vadeli trend göstergesi.',
    rsi:  'RSI (Göreceli Güç Endeksi, 0-100): 70 üzeri "aşırı alım", 30 altı "aşırı satım" sinyali kabul edilir.',
    pos52: 'Fiyatın son 52 haftalık en düşük–en yüksek aralığında nerede olduğu. %100 = zirvede.',
    vol:  'Seçilen dönemde işlem gören toplam miktar (likidite göstergesi).',
    mcap: 'Piyasa Değeri: dolaşımdaki toplam değer (fiyat × arz).',
};
                ...
                <div className="animate-fade-up">
                    ...
                    <div className="relative border mb-3.5" style={TS}>
                        ...
                        <div className="flex items-center gap-3.5 p-4">
                            <div className="min-w-0">
                                <div className="flex items-center gap-2">
                                    <h1 className="font-extrabold text-2xl leading-none" style={{ color: 'var(--color-text-primary)' }}>{data.name}</h1>
                                    <button type="button" onClick={() => toggle(symbol)} aria-label="Yıldızla">
                                        <Star className="w-[18px] h-[18px]" fill={starred ? '#f59e0b' : 'none'} style={{ color: '#f59e0b' }} />
                                    </button>
                                </div>
                                <div className="font-mono text-[12px] mt-1.5" style={{ color: 'var(--color-text-muted)' }}>{symbol}</div>
                            </div>
                            <div className="ml-auto text-right">
                                <div className="font-mono font-black text-3xl" style={{ color: 'var(--color-text-primary)' }}>{fmt(data.price, data.currency)}</div>
                                <span className="font-mono text-[14px] font-bold inline-flex items-center gap-1" style={{ color: chgColor }}>
                        ...
                        <div className="flex gap-1 px-3 pb-2">
                            {RANGES.map(([k, lbl]) => (
                                <button key={k} type="button" onClick={() => setRange(k)}
                                        className="font-mono text-[12px] font-bold px-3.5 py-1.5 border transition-colors"
                                        style={range === k
                        ...
                        <div className="px-2 pb-3">
                            ...
                            {rsi != null && (
                                <div className="px-2 mt-3">
                                    <div className="flex justify-between font-mono text-[10px]" style={{ color: 'var(--color-text-muted)' }}>
                                        <span className="inline-flex items-center">RSI (14)<InfoTip text={TIP.rsi} /></span>
                                        <span style={{ color: 'var(--color-text-primary)', fontWeight: 700 }}>{rsi} · {rsiSignal}</span>
                                    </div>
                        ...
                        <div className="grid grid-cols-4 border-t" style={{ borderColor: 'var(--color-terminal-border-raw)' }}>
                            {[
                                ...
                                ['HACİM', fmtBig(data.volume, data.currency), TIP.vol],
                                ['52H YÜKSEK', fmt(data.week52_high, data.currency)],
                                ['52H DÜŞÜK', fmt(data.week52_low, data.currency)],
                                ['PİYASA DEĞ.', fmtBig(data.market_cap, data.currency), TIP.mcap],
                            ].map(([l, v, tip], i) => (
                                <div key={l} className="px-3 py-2.5"
                                    ...
                                    <div className="font-mono text-[10px] inline-flex items-center" style={{ color: 'var(--color-text-muted)' }}>
                                        {l}{tip && <InfoTip text={tip} />}
                                    </div>
                                    <div className="font-mono text-[13.5px] font-bold mt-0.5" style={{ color: 'var(--color-text-secondary)' }}>{v}</div>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <Panel title="DÖNEM PERFORMANSI">
                        <div className="flex">
                            {[['1 HAFTA', perf.w1], ['1 AY', perf.m1], ['1 YIL', perf.y1], ['52H KONUM', perf.pos52 != null ? perf.pos52 : null]].map(([l, v], i) => (
                                <div key={l} className="flex-1 text-center py-3.5"
                                     style={{ borderRight: i < 3 ? '1px solid var(--color-terminal-border-raw)' : 'none' }}>
                                    <div className="font-mono text-[10px] inline-flex items-center justify-center" style={{ color: 'var(--color-text-muted)' }}>
                                        {l}{l === '52H KONUM' && <InfoTip text={TIP.pos52} />}
                                    </div>
                                    <div className="font-mono text-lg font-extrabold mt-1"
                    ...
                    <Panel title="TEKNİK GÖSTERGELER">
                        <div className="flex flex-wrap gap-2.5 p-3.5">
                            {[
                                ['MA20', data.ma20 != null && data.price >= data.ma20 ? '↑ fiyat üstte' : '↓ fiyat altta', TIP.ma20],
                                ['MA50', data.ma50 != null && data.price >= data.ma50 ? '↑ fiyat üstte' : '↓ fiyat altta', TIP.ma50],
                                ['RSI(14)', rsi != null ? `${rsi} · ${rsiSignal}` : '—', TIP.rsi],
                            ].map(([k, v, tip]) => (
                                <span key={k} className="font-mono text-[13px] px-3 py-2 border inline-flex items-center"
                                      style={{ borderColor: 'var(--color-terminal-border-raw)', background: 'rgba(16,185,129,0.05)', color: 'var(--color-text-secondary)' }}>
                                    {k} <b className="ml-1" style={{ color: 'var(--color-brand-primary)' }}>{v}</b>
                                    <InfoTip text={tip} />
                                </span>
                            ))}
                        </div>
                    ...
                    <div className="relative border mb-3.5" style={{ ...TS, borderLeft: '3px solid var(--color-brand-primary)' }}>
                        ...
                        <p className="px-4 py-3 text-[15px] leading-relaxed" style={{ color: 'var(--color-text-secondary)' }}>{data.summary}</p>
                    </div>

                    {/* AI yorumu */}
                    <div className="relative border" style={{ ...TS, borderLeft: '3px solid rgba(124,58,237,0.55)' }}>
                        <Corner />
                        <div className="flex items-center gap-2 font-mono text-[11px] font-bold tracking-widest px-4 py-3 border-b"
                             style={{ color: '#7c3aed', borderColor: 'var(--color-terminal-border-raw)' }}>
                            <Sparkles className="w-3.5 h-3.5" /> AI PİYASA YORUMU
                        </div>
                        {ai?.available ? (
                            <p className="px-4 py-3 text-[15px] leading-relaxed" style={{ color: 'var(--color-text-secondary)' }}>{ai.text}</p>
                        ) : (
