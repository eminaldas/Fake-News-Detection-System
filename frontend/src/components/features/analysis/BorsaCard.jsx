...
function Tile({ label, unit, value, change, decimals, loading }) {
    ...
    return (
        <div
            ...
            style={{ background: 'rgba(0,0,0,0.20)', border: '1px solid rgba(63,255,139,0.10)' }}
        >
            <div className="flex items-center justify-between mb-2">
                ...
                {chg !== null && (
                    <span className={`flex items-center gap-0.5 text-[9px] font-bold rounded-full px-1.5 py-0.5
                        ${isUp   ? 'bg-es-primary/20 text-es-primary' :
            ...
            {loading ? (
                ...
                <span className="text-xl font-manrope font-black text-white leading-none">
                    {unit}{Number(value).toLocaleString('tr-TR', { minimumFractionDigits: decimals, maximumFractionDigits: decimals })}
                </span>
            ) : (
                <span className="text-xl font-manrope font-black text-white/30">—</span>
            )}
        </div>
    );
}
...
const BorsaCard = () => {
    ...
    return (
        <div
            ...
            style={{
                background: 'linear-gradient(135deg, rgba(10,30,80,0.95) 0%, rgba(5,18,55,0.98) 100%)',
                border:     '1px solid rgba(84,224,253,0.18)',
                boxShadow:  '0 8px 32px rgba(84,224,253,0.06), 0 2px 8px rgba(0,0,0,0.4)',
            }}
        >
            {/* Dekoratif glow */}
            ...
            <div className="px-5 pt-5 pb-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <TrendingUp className="w-4 h-4" style={{ color: '#54e0fd' }} />
            ...
            <div className="px-5 pb-3 text-right">
                <span className="text-[9px] text-white/20 uppercase tracking-widest">truncgil.com</span>
            </div>
        </div>
    );
};
