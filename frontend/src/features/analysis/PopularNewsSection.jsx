...
function NewsCard({ item, rank }) {
    ...
    return (
        ...
        >
            ...
            {/* Rank badge */}
            <div className="absolute top-2.5 left-2.5 z-10 w-6 h-6 flex items-center justify-center"
                 style={{ border: `1px solid ${BRAND}`, background: 'rgba(0,0,0,0.6)' }}>
                <span className="font-mono font-bold text-[10px]" style={{ color: BRAND }}>{rank}</span>
            </div>

            {/* Kategori */}
            {item.category && (
                <span className="absolute top-2.5 right-2.5 z-10 font-mono text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5"
...
export default function PopularNewsSection() {
    ...
    return (
        <section className="w-full max-w-[1400px] mx-auto px-4 md:px-6 pb-12 mt-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 auto-rows-[220px]">
                {loading
                    ? Array.from({ length: 5 }).map((_, i) => <SkeletonCard key={i} />)
                    : news.map((item, i) => <NewsCard key={item.id} item={item} rank={i + 1} />)
                }
            </div>
        </section>
    );
